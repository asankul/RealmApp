...
class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let title = task.isComplete ? "Undone" : "Done"
        ...
        let doneAction = UIContextualAction(style: .normal, title: title) { [unowned self] _, _, isDone in
            if !task.isComplete {
                ...
                let rowIndex = IndexPath(row: completedTasks.count - 1, section: 1)
    ...
    private func showAlert(with task: Task? = nil, completion: (() -> Void)? = nil) {
        ...
        alert.action(with: task) { [weak self] taskTitle, note in
            if let task = task, let completion = completion {
                StorageManager.shared.edit(task, newTitle: taskTitle, note: note)
                completion()
            } else {
